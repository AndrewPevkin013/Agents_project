...
from app.engine.document_router import DocumentRouter
from app.engine.system_logger import SystemLogger
...
class AgentExecutor:
    ...
    def _create_agent(
        ...
        metadata = {
            ...
            "model_name": command.get(
                "model_name",
                "",
            ),
            "model_path": command.get(
                "model_path",
                "",
            ),
            "generation": command.get(
                "generation",
                {
                    "max_new_tokens": 256,
                    "temperature": 0.7,
    ...
    def _split(self, command: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for name in target_agents:
            self.registry.upsert_from_metadata({
                ...
                "type": "llm",
                "description":
                    f"Specialized agent split from {source_agent}",
                "system_prompt":
                    source_meta.get("system_prompt", ""),
                "tags":
                    source_meta.get("tags", []),
                "model_name":
                    source_meta.get("model_name", ""),
                "model_path":
