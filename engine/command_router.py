...
from engine.handler import HandlerOutput
...
class CommandRouter:
    ...
    def route_by_agent_name(self, agent_name: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        return self.route({"type": "agent_call", "agent": agent_name, "payload": payload})
    def handle_user_request(self, user_request: str) -> Dict[str, Any]:
        if self.handler is None:
            raise RuntimeError("Handler is not configured")
        handler_output: HandlerOutput = self.handler.handle(user_request)
        if isinstance(handler_output, str):
            return {"user_request": user_request, "mode": "system_qa", "commands": [], "engine_response": [], "final_answer": handler_output}
        commands = handler_output
        engine_response = self.route_many(commands)
        return {"user_request": user_request, "mode": "command", "commands": commands, "engine_response": engine_response, "final_answer": self._compile_answer(commands, engine_response)}
    @staticmethod
    def _compile_answer(commands: List[Dict[str, Any]], engine_response: List[Dict[str, Any]]) -> str:
        ...
        return "Выполнено команд: " + str(len(commands)) + ". Результат: " + json.dumps(engine_response, ensure_ascii=False)
