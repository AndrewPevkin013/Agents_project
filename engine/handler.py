...
class RuleBasedHandler:
    ...
    def build_system_prompt(self, user_request: str) -> str:
        return build_handler_system_prompt(user_request, self.registry.as_prompt_registry(), self.command_store.retrieve)
    def handle(self, user_request: str) -> HandlerOutput:
        text, lower = user_request.strip(), user_request.strip().lower()
        if any(x in lower for x in ["какие агенты", "список агентов", "list agents", "what agents"]):
            return "Доступные агенты: " + ", ".join(self.registry.list_agents()) + ". Можно запустить агента или создать нового."
        ...
        if ("создай" in lower and "агент" in lower) or ("create" in lower and "agent" in lower):
            ...
            command = {"action": "create_agent", "agent": name, "type": "llm" if model_name else "mock", "description": f"Agent created from request: {text}", "system_prompt": f"Ты агент {name}. Твоя роль сформирована из запроса пользователя: {text}", "tags": self._guess_tags(text)}
            if model_name:
        ...
        return [{"action": "extract", "agents": [selected], "prompts": {selected: text}}]
    @staticmethod
    ...
    def _extract_model_name(text: str) -> str | None:
        for model in ["Qwen2.5-3B-Instruct", "Qwen2.5-7B-Instruct", "Qwen2.5-14B-Instruct", "Mistral-7B"]:
            if model.lower() in text.lower():
                return model
        return None
    @staticmethod
    def _extract_file_path(text: str) -> str:
        m = re.search(r"([A-Za-z]:[\\/][^\s]+|[\w./\\-]+\.(?:txt|md|json|pdf|csv|py))", text)
        return m.group(1) if m else ""
    @staticmethod
    def _guess_tags(text: str) -> List[str]:
        lower = text.lower(); mapping = {"backend": ["backend", "api", "server"], "api": ["backend", "api"], "бэкенд": ["backend", "api", "server"], "devops": ["devops", "docker", "deploy"], "docker": ["devops", "docker"], "деплой": ["devops", "deploy"], "test": ["testing", "qa"], "qa": ["testing", "qa"], "тест": ["testing", "qa"], "security": ["security"], "безопас": ["security"], "frontend": ["frontend", "ui"], "ui": ["frontend", "ui"], "analysis": ["analysis", "summary"], "аналит": ["analysis", "summary"]}
        tags = []
...
class LLMHandler:
    def __init__(self, registry: AgentRegistry, command_store: CommandStore, core_config_path: str | Path) -> None:
        self.registry, self.command_store, self.core_config_path = registry, command_store, Path(core_config_path)
    ...
    def _ensure_loaded(self) -> None:
        ...
        import torch
        torch_dtype = {"float32": torch.float32, "float16": torch.float16, "bfloat16": torch.bfloat16}.get(self.dtype_name, torch.float32)
        print(f"Loading CORE model: {self.model_path}"); print(f"CORE device: {self.device}, dtype: {self.dtype_name}")
    ...
    def build_system_prompt(self, user_request: str) -> str:
        return build_handler_system_prompt(user_request, self.registry.as_prompt_registry(), self.command_store.retrieve)
    def handle(self, user_request: str) -> HandlerOutput:
        self._ensure_loaded(); system_prompt = self.build_system_prompt(user_request)
        ...
        print("CORE raw answer:"); print(answer)
    ...
    def _parse_handler_output(text: str) -> HandlerOutput:
        ...
        try:
            ...
            return [parsed] if isinstance(parsed, dict) else parsed if isinstance(parsed, list) else text
...
def create_handler(registry: AgentRegistry, command_store: CommandStore, core_config_path: str | Path, use_llm: bool = True):
